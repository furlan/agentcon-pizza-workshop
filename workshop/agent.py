...
vector_store_id = "vs_MMJQhPtCXrLhTACmiGHCK2GM"
...
try:
    while True:
        ...
        # Check run status and error details
        print(f"Run status: {run.status}")
        
        if run.status == "failed":
            print(f"Error code: {run.last_error.code if run.last_error else 'None'}")
            print(f"Error message: {run.last_error.message if run.last_error else 'None'}")
            continue
        ...
        if assistant_message:
            response_text = next((item["text"]["value"] for item in assistant_message.content if item.get("type") == "text"), "")
